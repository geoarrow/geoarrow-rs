...
use crate::import::scalar::{
    GEOSConstLineString, GEOSConstLinearRing, GEOSConstPoint, GEOSConstPolygon,
...
macro_rules! impl_const_specialization {
    ($geometry_type:ident, $geometry_variant:ident) => {
...
impl_const_specialization!(GEOSConstPoint, Point);
impl_const_specialization!(GEOSConstLineString, LineString);
impl_const_specialization!(GEOSConstPolygon, Polygon);

impl<'a> GeometryTrait for GEOSConstLinearRing<'a> {
    type T = f64;
    type PointType<'b>
        = GEOSConstPoint<'a>
    where
