...
impl<G: LineStringTrait<T = f64>> From<(Vec<Option<G>>, LineStringType)> for LineStringArray {
    fn from(other: (Vec<Option<G>>, LineStringType)) -> Self {
        let mut_arr: LineStringBuilder = other.into();
        mut_arr.into()
    }
}

impl<G: LineStringTrait<T = f64>> From<(&[G], LineStringType)> for LineStringArray {
    fn from(other: (&[G], LineStringType)) -> Self {
        let mut_arr: LineStringBuilder = other.into();
        mut_arr.into()
    }
}
