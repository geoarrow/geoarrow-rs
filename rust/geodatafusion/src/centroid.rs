...
pub fn centroid_udf() -> ScalarUDF {
    let centroid = Arc::new(|args: &[ColumnarValue]| {
        ...
        Ok(ColumnarValue::from(
            GeometryArray::from(output).into_array_ref(),
        ))
    });
    ...
    create_udf(
        ...
        Arc::new(NativeType::Geometry(CoordType::Separated).to_data_type()),
...
mod test {
    ...
    async fn test() -> Result<()> {
        ...
        let sql_df = ctx.sql("SELECT centroid(geometry) FROM table;").await?;
        // print the results
        sql_df.show().await?;
