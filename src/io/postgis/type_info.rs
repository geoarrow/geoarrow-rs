...
impl PgTypeInfo {
    pub fn from_upstream(value: &sqlx::postgres::PgTypeInfo) -> Option<Self> {
        if let Some(oid) = value.oid() {
            Self::try_from_oid(Oid(oid.0))
        } else {
            None
        }
    }
...
pub enum PgType {
    ...
    Void,
    // Note(kyle):
    // We exclude the following variants because they will never be constructed from an OID value
...
impl PartialEq<PgType> for PgType {
    fn eq(&self, other: &PgType) -> bool {
        // Note(kyle) Updated because here we always have known oids
        self.oid() == other.oid()
    }
}
